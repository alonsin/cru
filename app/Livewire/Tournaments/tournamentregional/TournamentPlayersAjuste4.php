...
class TournamentPlayersAjuste4 extends Component
{
	...
	public function loadDataSubita1AjusteMain()
	{
		...
		$jugadoresCollection1  = TournamentPlayer::with('player')
			...
			->whereIn('NUM_4', Arr::flatten($this->enfrentamientos1))
			->get();
		...
		foreach ($this->jugadores1 as $jugador) {
			...
			if ($tp) {
				$this->sorteossubita1[$id] = $tp->NUM_4;
			}
		}
	}
		...
		foreach ($this->enfrentamientos1 as $index => $par) {
			...
			if (isset($this->jugadores1[$clave1]) && isset($this->jugadores1[$clave2])) {
				...
				if ($ganador1) {
					...
					$updateJugador1 = [
						'NUM_2' => $index + 1, // Asignar el número de enfrentamiento
					];
	...
	{
		return view('livewire.tournaments.tournamentregional.tournament-player-ronda-semifinal');
	}
}
