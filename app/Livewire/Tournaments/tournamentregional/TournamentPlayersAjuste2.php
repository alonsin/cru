...
class TournamentPlayersAjuste2 extends Component
{
	...
	public function loadDataSubita1AjusteMain()
	{
		...
		$jugadoresCollection1  = TournamentPlayer::with('player')
			...
			->whereIn('NUM_2', Arr::flatten($this->enfrentamientos1))
			->get();
		...
		foreach ($this->jugadores1 as $jugador) {
			...
			if ($tp) {
				$this->sorteossubita1[$id] = $tp->NUM_2;
			}
		}
	}
		...
		foreach ($this->enfrentamientos1 as $index => $par) {
			...
			if (isset($this->jugadores1[$clave1]) && isset($this->jugadores1[$clave2])) {
				...
				if ($ganador1) {
					...
					$updateJugador1 = [
						'R_FINAL' => 1, // Asignar el número de enfrentamiento
					];
	...
	{
		return view('livewire.tournaments.tournamentregional.tournament-player-ronda-final');
	}
}
